...
public class BarcodeScannerView extends FrameLayout implements Camera.PreviewCallback {
    ...
    private String descText = "请将下方矩形框对准包装或设备上的二维码进行识别扫描";
    ...
    public BarcodeScannerView(Context context) {
        ...
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT, Gravity.TOP | Gravity.CENTER_HORIZONTAL);
        ...
        params.leftMargin = (int) (scale * 55 + 0.5F);
        params.bottomMargin = (int) (scale * 100 + 0.5F);
        this.addView(cancel, params);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        desc.layout(rect.left, rect.top / 2, rect.right, rect.top * 2 / 3);

        int imageViewLeft = (rect.left + rect.right) / 2 - imageView.getWidth()/2;
        int viewTop = (getHeight() + rect.bottom) / 2;
        int imageViewTop = viewTop - imageView.getHeight() - (int) (scale * 20 + 0.5F);

        cancel.layout(rect.left - cancel.getWidth(), viewTop, rect.left, viewTop + cancel.getHeight());
